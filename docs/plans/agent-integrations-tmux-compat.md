...
cmux Settings list (Claude Code Teams, oh-my-opencode, oh-my-codex, oh-my-pi, oh-my-claudecode) **not** same as lifecycle hooks.
...
Agents (Claude Teams, OMO, OMX, OMC) spawn worker panes via **tmux**. cmux no reimplements orchestration. **Shims tmux**:
...
**Hooks role in teams:** each teammate pane = normal PTY; lifecycle hooks fire Stop/Notification → badges. Without hooks: splits work, no attention signals. Without tmux-compat: hooks only on single pane, no multi-agent grid.
...
New PTYs get `RMUX_*` → hooks keep working in teammate panes.
...
1. **API:** optional `pane_id` on `surface.send_text` (prefer write to target pane without prior focus if easy).
...
- Document: `hooks setup` required for team notifications
...
docs/plans/agent-integrations-tmux-compat.md  # this file
